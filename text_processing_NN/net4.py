...
MAX_REGULARIZER = 0.00045
REGULARIZER_STEP = 0.00003
OUTPUT_JSON_FILE = "training_data_output/4_layers2.json"
...
def build_model(regularizer_strength):
    ...
    x = Dense(256, activation="relu", kernel_regularizer=l2(regularizer_strength), name="dense_1_l2")(x)
    x = tf.keras.layers.Dropout(DROPOUT)(x)
    x = Dense(128, activation="relu", kernel_regularizer=l2(regularizer_strength), name="dense_2_l2")(x)
    x = tf.keras.layers.Dropout(DROPOUT)(x)
    x = Dense(64, activation="relu", kernel_regularizer=l2(regularizer_strength), name="dense_3_l2")(x)
    x = tf.keras.layers.Dropout(DROPOUT)(x)
    x = Dense(32, activation="relu", kernel_regularizer=l2(regularizer_strength), name="dense_4_l2")(x)
...
while current_regularizer <= MAX_REGULARIZER:
    ...
    print("\n" + "="*70)
    print(f"--- Training with L2 Regularizer: {current_regularizer:.6f} ---")
    print("="*70)
    ...
    result_entry = {
        "l2_regularizer": f"{current_regularizer:.6f}", 
